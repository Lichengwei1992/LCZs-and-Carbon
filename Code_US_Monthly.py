...
先完成美国，再完成中国，共用同一套流水线代码。
  ...
  [CO2]  月度文件：{prefix}_{YYMM:04d}.tif
         YYMM = (year-2000)*100 + month  （即年份后两位 × 100 + 月份）
         例：2000年1月 → 0001，2001年3月 → 0103，2019年12月 → 1912
         美国路径: D:/LCZCarbon/US/{year}/USA_odiac2024_1km_excl_intl_{YYMM:04d}.tif
         中国路径: D:/LCZCarbon/CHN/{year}/CHN_odiac2024_1km_excl_intl_{YYMM:04d}.tif

  [GHI]  月度独立文件（无需拼接）：
         美国: D:/LCZCarbon/GHI/USA_GHI_Monthly/USA-{year}/USA_{year}_{month:02d}.tif
         中国: D:/LCZCarbon/GHI/CHN_GHI_Monthly/China-{year}/GHI_{year}_{month:02d}.tif
  ...
  Model 3 - Seasonal:   + Season x LCZ 交互项
...
CONFIGS = {
    'USA': {
        ...
        # 月度 GHI：D:\LCZCarbon\GHI\USA_GHI_Monthly\USA-{year}\USA_{year}_{month:02d}.tif
    ...
    'CHN': {
        ...
        # 月度 GHI：D:\LCZCarbon\GHI\CHN_GHI_Monthly\China-{year}\GHI_{year}_{month:02d}.tif
...
def co2_filename(prefix, year, month):
    """
    生成CO2月度文件名
    YYMM = (year-2000)*100 + month，即年份后两位拼上两位月份，零填充至4位
    例: 2000-01 -> 0001, 2001-03 -> 0103, 2019-12 -> 1912
    """
...
def run_monthly_regression(df_input, fe_col, out_dir, label):
    ...
    Model 1: Baseline
    Model 2: + LCZ 主效应（基准: LCZ9）
    Model 3: + Season x LCZ 交互项（Spring 为基准季节）

    Month FE 系数含义：控制其他变量后，各月相对基准月（1月）的 CO2 系统性差异。
    Season x LCZ 交互系数含义：某 LCZ 类型在特定季节相对 Spring 的额外碳排放效应。
    """
    df = df_input.copy()
    ...
    for name, formula in models_def.items():
        try:
            ...
            with open(os.path.join(out_dir, f"{name}_Summary.txt"), 'w',
                      encoding='utf-8') as f:
                f.write(f"面板: 像元 x 年月 | FE: {fe_col} + Year + Month\n")
...
def run_country(country, cfg):
    ...
    if not os.path.exists(ref_co2):
        raise FileNotFoundError(
            f"找不到基准CO2文件: {ref_co2}\n"
            f"请检查路径 co2_base='{cfg['co2_base']}' 下是否有 2000 子文件夹"
        )
    with rasterio.open(ref_co2) as src:
        ref_meta = src.meta.copy()
    print(f"  基准栅格: {ref_meta['width']} x {ref_meta['height']} px, "
          f"CRS={ref_meta['crs']}")
    ...
    for year in YEARS:
        ...
        for month in MONTHS:
            ...
            # 检查必需文件
            missing = [(p, lb) for p, lb in [(f_co2, 'CO2'), (f_ghi, 'GHI')]
    ...
    print(f"  合并: {len(df_all):,} 行，"
          f"{df_all['Year'].nunique()} 年 x {df_all['Month'].nunique()} 月")
